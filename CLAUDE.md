...
- **Content & MDX**: `@next/mdx` is wired in [next.config.ts](next.config.ts) (`pageExtensions` includes `mdx`), but MDX bodies are actually rendered through `next-mdx-remote/rsc` (`MDXRemote`). Frontmatter is parsed with `gray-matter` and validated with `zod` (see [lib/mdx.ts](lib/mdx.ts)). Code fences are rewritten before rehype runs by the remark plugin `remarkPierreCode` ([lib/remark-pierre-code.ts](lib/remark-pierre-code.ts)), which turns each non-mermaid fence into a `CodeFile`/`PatchDiff`/`MultiFileDiff` JSX element rendered by `@pierre/diffs`-backed components in [components/diff/](components/diff/). The remaining rehype pipeline is `rehypeSlug` → `rehypeMermaid`, registered identically on both `app/blog/[slug]/page.tsx` and `app/project/[slug]/page.tsx`.
...
- Each post type owns its own `layout.tsx`, but both share the identical structure — a 12-column grid with a sticky `OnThisPage` aside — via the shared `PostLayout` server component ([components/post-layout.tsx](components/post-layout.tsx)). `app/blog/[slug]/layout.tsx` and `app/project/[slug]/layout.tsx` are thin wrappers around it (App Router requires a `layout.tsx` per route subtree). Change the shared layout in `components/post-layout.tsx`, not in the route files.
...
- **[lib/remark-pierre-code.ts](lib/remark-pierre-code.ts)** — `remarkPierreCode`: a remark plugin (runs before rehype) that visits every mdast `code` node except `lang === "mermaid"` and rewrites it into an MDX JSX element — `CodeFile` (plain/tabbed code), `PatchDiff` (` ```diff `/` ```patch ` fences), or `MultiFileDiff` (` ```multidiff ` fences, split on a `===`-style separator line) — based on fence language, so no raw `<pre><code>` for non-mermaid fences ever reaches the hast tree.
- **[lib/diffs-theme.ts](lib/diffs-theme.ts)** — theme wiring for `@pierre/diffs`.
- **[components/diff/](components/diff/)** — the 9 components that render `remarkPierreCode`'s output: `CodeFile`, `CodeTabs`/`CodeTabsClient`, `PatchDiff` (`code-patch.tsx`), `MultiFileDiff` (`code-multi.tsx`), plus the shared `File`/`DiffCopyButton` primitives (`file.tsx`, `diff-copy-button.tsx`).
- **[lib/rehype-mermaid.ts](lib/rehype-mermaid.ts)** — `rehypeMermaid`: finds ```` ```mermaid ```` fences and replaces the `<pre>` with a pre-rendered SVG produced by `beautiful-mermaid`'s `renderMermaidSVG`. Background/foreground are wired to CSS vars so dark mode picks them up. Registered identically on both the blog and project routes ([app/blog/[slug]/page.tsx](app/blog/[slug]/page.tsx), [app/project/[slug]/page.tsx](app/project/[slug]/page.tsx)).
...
- **[mdx-components.tsx](mdx-components.tsx)** — `getMDXComponents()` returns the component map handed to every `MDXRemote`. It overrides `h1`–`h3`, `a` (with three branches: internal `Link`, anchor, external with `ArrowUpRight`), `blockquote`, `figure`/`figcaption` (passthrough, used for image captions), `pre`, `code` (inline vs block via `data-language` sniff), and exposes three custom MDX-only components: `File`, `Url`, `DeepDive` (a `Collapsible`-based expandable section), plus `CodeGroup`.
